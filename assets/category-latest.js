(function(){
  ...
  section.innerHTML='<div class="cln-h"><h2>'+(english?'Latest <em>updates</em>':'Tin <em>mới cập nhật</em>')+'</h2><div class="cln-line"></div><a class="cln-all" href="'+(english?'/en/news':'/news')+'">'+(english?'View all':'Xem tất cả')+'</a></div><div class="cln-grid">'+stories[category].map(function(s){var ext=s[0]==='playstation-plus-august-2026'?'.jpg':'.svg';return '<a class="cln-card" href="'+(english?'/en/':'/')+s[0]+'"><img src="/assets/img/news-'+s[0]+ext+'" alt="'+(english?s[2]:s[1]).replace(/"/g,'&quot;')+'" loading="lazy" width="1200" height="675"><div class="cln-copy"><div class="cln-cat">'+category+'</div><div class="cln-title">'+(english?s[2]:s[1])+'</div></div></a>'}).join('')+'</div>';
